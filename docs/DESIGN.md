...
* **ファイル特定**: `--data-dir` 配下の `{YYYYMMDDHHmm}{sensor_id}.spkx`（UTC）。**各分について宣言済み全センサーのファイル**を読み、`RawSensorEvent` の絶対時刻でマージソートしてから投入する（複数センサーを跨いでもウォーターマークの単調性を保つため）。欠損ファイルは警告計数してスキップする（その分は DELETE のみ実行され DB 上は空になる）。
