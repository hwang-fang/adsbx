...
pub struct Deduplicator {
    ...
    /// 失効時刻(100ns) -> その時刻に失効するフレーム群。
    expiry: BTreeMap<i64, Vec<ModeSFrame>>,
}

impl Deduplicator {
    pub fn new(ttl_ms: u32) -> Self {
        Self {
            ttl_100ns: ttl_ms as i64 * 10_000,
            seen: HashSet::new(),
            expiry: BTreeMap::new(),
        }
    }
    ...
    pub fn accept(&mut self, frame: ModeSFrame, timestamp_100ns: i64) -> bool {
        if !self.seen.insert(frame) {
            return false;
        }
        let expire_at = timestamp_100ns.saturating_add(self.ttl_100ns);
        self.expiry.entry(expire_at).or_default().push(frame);
        true
    }

    /// ウォーターマーク `T` までに失効したビット列を HashSet からパージする。
    pub fn purge(&mut self, watermark_100ns: i64) {
        // split_off(&k) は k 以降を残すので、k = watermark+1 で「<= watermark」を取り出す。
        // 終端ドレインの Watermark(i64::MAX) でも溢れないよう saturating。
        let mut expired = self.expiry.split_off(&watermark_100ns.saturating_add(1));
...
mod tests {
    ...
    fn first_come_wins_duplicate_dropped() {
        let mut d = Deduplicator::new(50);
        assert!(d.accept(f(1), 1000));
        // 同一ビット列が TTL 窓内に到着 -> 破棄。
        assert!(!d.accept(f(1), 1200));
        // 別ビット列は通過。
        assert!(d.accept(f(2), 1300));
    }

    #[test]
    fn purge_releases_after_ttl() {
        let mut d = Deduplicator::new(50); // 50ms = 500_000 (100ns)
        d.accept(f(1), 1_000_000);
        ...
        assert!(d.accept(f(1), 2_000_000));
    }

    #[test]
    fn purge_with_max_watermark_does_not_overflow() {
        // 終端ドレインで送られる Watermark(i64::MAX) でパニックしないこと。
        let mut d = Deduplicator::new(50);
        d.accept(f(1), 1_000_000);
        d.purge(i64::MAX);
        assert_eq!(d.live_len(), 0);
    ...
    fn no_memory_leak_after_full_purge() {
        let mut d = Deduplicator::new(50);
        for i in 0..1000 {
            d.accept(f((i % 250) as u8), 1_000_000 + i as i64);
        }
        d.purge(i64::MAX / 2);
        assert_eq!(d.live_len(), 0);
